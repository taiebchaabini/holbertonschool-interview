...
void sandpiles_toppling(int (*grid1)[3], int grid2[3][3])
{
    ...
    while (y < 3)
    {
        ...
        while (x < 3)
        {
            /** Checks the old X and remove 4 to new grid if the value is more than 3 **/
            if (grid2[y][x] > 3)
            {
                (*grid1)[x] -= 4;
            }
            /** Checks the left and add +1 if the value is more than 3 **/
            if (x - 1 >= 0)
            {
                if (grid2[y][x - 1] > 3)
                    (*grid1)[x] += 1;
            }
            /** Checks the right value add +1 if the value is more than 3 **/
            if (x + 1 < 3)
            {
                if (grid2[y][x + 1] > 3)
                    (*grid1)[x] += 1;
            }
            /** Checks the top value add +1 if the value is more than 3 **/
            if (y - 1 >= 0)
            {
                if (grid2[y - 1][x] > 3)
                    (*grid1)[x] += 1;
            }
            /** Checks the bottom value add +1 if the value is more than 3 **/
            if (y + 1 < 3)
            {
                if (grid2[y + 1][x] > 3)
                    (*grid1)[x] += 1;
